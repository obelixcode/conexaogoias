...
export function BannerDisplay({ banners, className = '', variant = 'default' }: BannerDisplayProps) {
  ...
  const handleImageError = (bannerId: string) => {
    console.error('Erro ao carregar imagem do banner:', bannerId);
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      {banners.map((banner) => {
        ...
        const hasInvalidUrl = !isValidImageUrl(banner.image);
        const isFirebaseUrl = isFirebaseStorageUrl(banner.image);
            ...
            <div className={`relative w-full ${getHeight()} overflow-hidden`}>
              {hasImageError || hasInvalidUrl ? (
                ...
                <Image
                  ...
                  onError={() => {
                    console.log(`Erro ao carregar imagem do banner ${banner.id}:`, banner.image);

...
# Script para remover dependências problemáticas do WordPress
# Execute: ./scripts/remove-wordpress-deps.sh

set -e

echo "🗑️ Removendo dependências problemáticas do WordPress..."
...
print_status "Fazendo backup do package.json..."
cp package.json package.json.backup

# Remover dependências problemáticas do WordPress
print_status "Removendo dependências problemáticas do WordPress..."
npm uninstall @wordpress/block-editor @wordpress/components @wordpress/commands @wordpress/preferences @wordpress/data @wordpress/element @wordpress/hooks @wordpress/i18n @wordpress/keycodes @wordpress/notices @wordpress/plugins @wordpress/rich-text @wordpress/url @wordpress/utils @wordpress/wordcount --save

# Remover dependências do react-spring que causam conflitos
print_status "Removendo dependências do react-spring..."
npm uninstall @react-spring/animated @react-spring/core @react-spring/web @react-spring/shared --save

# Limpar dependências
print_status "Limpando dependências..."
rm -rf node_modules package-lock.json

# Criar .npmrc mais agressivo
...
if npm run build; then
    ...
    print_status "Dependências problemáticas removidas! 🎉"
    print_warning "⚠️  Nota: Funcionalidades do WordPress removidas para resolver conflitos"
else

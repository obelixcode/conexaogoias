...
# Script para resolver conflitos de dependências WordPress/React
# Execute: ./scripts/fix-wordpress-deps.sh

set -e

echo "🔧 Resolvendo conflitos de dependências WordPress/React..."
...
npm uninstall @wordpress/block-editor @wordpress/components @wordpress/commands @wordpress/preferences --save
...
# Instalar dependências principais sem WordPress
...
if npm run build; then
    ...
    print_status "Conflitos de dependências WordPress resolvidos! 🎉"
    print_warning "⚠️  Nota: Algumas funcionalidades do WordPress podem não estar disponíveis"
else
